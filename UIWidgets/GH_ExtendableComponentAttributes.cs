...
namespace KarambaUIWidgets.UIWidgets
{
	...
	public class GH_ExtendableComponentAttributes : GH_ComponentAttributes
	{
        ...
        //                                                                                                                                                  
        //                                                                                                                                                  
        private RectangleF _FloatingThumbnailBounds;
		private int AllAroundSpacing = 5;
        public Bitmap ThumbnailImage { get; set; }

        //This is the method that will create the floating thumbnail below the component.
        private void DrawFloatingThumbnail(GH_Canvas canvas)
        {

            // Get the component bounds as a starting reference point.
            RectangleF ComponentBounds = Bounds;

            // Define the size and position of the floating thumbnail.
            float Gap = 20f;       // gap between component bottom and thumbnail
            float Width = ComponentBounds.Width;  // thumbnail width is same as the component width
			float Height = ComponentBounds.Width; // default is square shape.

            if (ThumbnailImage != null)
			{
				float ImageAspectRatio = (float)ThumbnailImage.Width / ThumbnailImage.Height;
                Height = Width / ImageAspectRatio;  // thumbnail size is half the component width
			}

			float X = ComponentBounds.X;  //X-coordinate to place it centered horizontally.
            float Y = ComponentBounds.Bottom + Gap;  //Y-coordinate to place it below the component.

            _FloatingThumbnailBounds = new RectangleF(X, Y, Width, Height);

            // Draw the thumbnail capsule.
            using (var capsule = GH_Capsule.CreateCapsule(_FloatingThumbnailBounds, GH_Palette.Blue))
            {
                capsule.Render(canvas.Graphics, Selected, Owner.Locked, false);
            }

            // Draw image inside capsule if it exists
            if (ThumbnailImage != null)
            {
                RectangleF ImageBounds = RectangleF.Inflate(_FloatingThumbnailBounds, -AllAroundSpacing, -AllAroundSpacing);
                canvas.Graphics.DrawImage(ThumbnailImage, ImageBounds);
            }
        }
        ...
        protected override void Render(GH_Canvas iCanvas, Graphics graph, GH_CanvasChannel iChannel)
		{
			...
			else
			{
                ...
                //FloatingThumbnail rendering.
				DrawFloatingThumbnail(iCanvas); //GH_CanvasChannel.Objects is the layer of the component where the floating thumbnail will be drawn and rendered.
